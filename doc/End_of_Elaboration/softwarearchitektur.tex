...
\begin{tabulary}{\linewidth}{lLLLLl}
	\toprule
	    ...
	    \bfseries Enum-Wert\\
	\midrule
  % FIXME Enum-Werte aktualisieren (insbesondere Fall 3: NEW)
...
% FIXME override -> overwrite (schon geändert, aber wo haben wir das sonst noch falsch?)
%
% override: Überschreiben im Sinne von "einen Fehler ignorieren" oder
% "einen Sicherheitsmechanismus ausser Kraft setzen"
%
% overwrite: Eine Datei überschreiben
...
Wir haben uns deswegen für eine Plugin-Architektur entschieden, so dass neue Plattformen einfach als neue Plugins geschrieben werden können, die dann in \emph{kitovu} eingehängt werden (via dem \emph{Template Method}-Pattern). So ist es theoretisch möglich, eine Vielzahl von externen Plattformen in \emph{kitovu} zu integrieren, ohne dass der gesamte Code umgeschrieben werden muss.

Nachdem sich die ursprüngliche Plugin-Architektur \verb|pluggy| als unbrauchbar
erwiesen hatte, entschieden wir uns für
\verb|stevedore|.\footnote{\url{https://docs.openstack.org/stevedore/latest/}}.
Wir haben pluggy verworfen, da es eine andere Architektur für Plugins
...
Plugins aufgerufen wird (\emph{Hooks}, ähnlich dem \emph{Observer}-Pattern). Im Kontrast dazu
...
Stevedore unterstutzt dies, und nennt es ein ''Driver`` pattern: \\
